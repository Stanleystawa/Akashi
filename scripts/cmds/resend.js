...
module.exports = {
        ...
        if(event.type == "message_unsend") {
          ...
          const name = await usersData.getName(senderID);
            if(getMsg.attachment[0] == undefined) {
                api.sendMessage(`${name} remove this message: ${getMsg.msgBody}`,threadID);
            } else {
                let num = 0
                let msg = {
                    body:`${name} just removed ${getMsg.attachment.length} Attachments.${(getMsg.msgBody != "") ? `\\ontent: ${getMsg.msgBody}` : ""}`,
                    attachment:[],
    ...
    onStart: async function({ api, event, threadsData}) {
        ...
        fs.writeFileSync("./resend_data.json", JSON.stringify(data, null, 10));

        return api.sendMessage(`Successfully ${(data[threadID] == true) ? "turned on" : "turned off"}!`, threadID, messageID);
    }
};