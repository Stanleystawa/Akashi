...
const a = "repl";
      const res = await axios.get(`https://sandyapi.otinxsandeep.${a}.co/liner?prompt=${encodeURIComponent(prompt)}`);
...
You can reply to continue chatting`,
  ...
  onReply: async function ({ message, event, Reply, args, api, usersData }) {
    try {
      ...
      const res = await axios.get(`https://otinxsandip.onrender.com/liner?prompt${encodedPrompt}`);
      const result = res.data.answer;
